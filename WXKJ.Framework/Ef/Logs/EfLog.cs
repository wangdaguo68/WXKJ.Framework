...
namespace WXKJ.Framework.Datas.Ef.Logs
{
    /// <summary>
    /// Ef日志记录器
    /// </summary>
    public class EfLog : ILogger {
        ...
        protected virtual ILog GetLog() {
            try {
                return WXKJ.Framework.Logs.Log.GetLog( TraceLogName );
            }
            catch {
                return WXKJ.Framework.Logs.Log.Null;
            }
        }
        ...
        private void AddSql( string sql, ILog log ) {
            if( string.IsNullOrWhiteSpace( sql ) )
                return;
            log.Sql( $"{sql}{Environment.NewLine}" );
        }
