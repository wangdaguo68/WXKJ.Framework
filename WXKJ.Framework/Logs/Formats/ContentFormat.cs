...
using WXKJ.Framework.Logs.Contents;
using WXKJ.Framework.Logs.Properties;
using WXKJ.Framework.Util.Extensions;

namespace WXKJ.Framework.Logs.Formats
{
    /// <summary>
    /// 内容格式化器
    /// </summary>
    public class ContentFormat : ILogFormat {
        ...
        /// 获取异常类型列表
        /// </summary>
        private static string GetExceptionTypes( Exception exception ) {
        ...
        {
            return string.IsNullOrWhiteSpace( errorCode ) ? string.Empty : $"-- {LogResource.ErrorCode}: {errorCode}";
        }
